...
export default function Leaderboard() {
  ...
  const ltpUsed = leaderboard.ltp_used ?? 0;
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center gap-3 px-3 py-1.5 bg-slate-50 dark:bg-[#0d1117] border border-slate-200 dark:border-[#1e293b]/60 rounded-lg">
          ...
          <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
            Index LTP: <span className="text-slate-700 dark:text-slate-200 font-mono">${ltpUsed.toFixed(2)}</span>
          </span>
        </div>
      </div>
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div className="flex flex-col items-end min-w-[80px]">
                    ...
                    <div className="flex items-center gap-1">
                       <span className={`text-[13px] font-mono font-bold ${pnl >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
                          {pnl >= 0 ? '+' : ''}{pnl.toFixed(2)}
                       </span>
                    </div>
                  </div>
